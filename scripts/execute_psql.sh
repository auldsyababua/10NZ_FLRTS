...
# ==========================================
# 10NetZero-FLRTS: Execute Schema via psql
# ==========================================
# Version: 1.0
# Date: May 21, 2025
# Description: Direct connection to Supabase PostgreSQL using psql

# Set PostgreSQL connection variables
export PGHOST="db.thnwlykidzhrsagyjncc.supabase.co"
export PGPORT="5432"
export PGUSER="postgres"
export PGPASSWORD="lYDPaAqABpnu6dCA"
...
SCHEMA_FILE="supabase_schema_fixed.sql"
UPDATE_FILE="supabase_schema_update.sql"
SAMPLE_DATA_FILE="supabase_sample_data_upsert.sql"

# Create fixed schema file with correct table ordering
cat > "$SCHEMA_FILE" << 'EOF'
-- ==========================================
...
execute_sql_file() {
  ...
  echo "Executing $description: $file"
  echo "----------------------------------------------"
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "Would you like to continue to the next file? (y/n)"
...
# Main execution
echo "Starting database schema execution..."

# Execute the main schema
execute_sql_file "$SCHEMA_FILE" "Main Schema"

# Execute the schema updates
execute_sql_file "$UPDATE_FILE" "Schema Updates"

# Execute the sample data if file exists
if [ -f "$SAMPLE_DATA_FILE" ]; then
  execute_sql_file "$SAMPLE_DATA_FILE" "Sample Data"
fi
